...
if (isset($_POST['data_sched']) && isset($_POST['room_data']) && isset($_POST['teach_load_data']) && isset($_POST['subject_data'])) {
    ...
    $title = $_POST['title'];
    isset($_POST['first_break']) ? $_POST['first_break'] : $_POST['first_break'] = 0;
    $first_break = $_POST['first_break'];
    isset($_POST['second_break']) ? $_POST['second_break'] : $_POST['second_break'] = 0;
    $second_break = $_POST['second_break'];
    ...
    if ($first_break == "" || $second_break == "") {
        $error_message = "Break time is required";
        $res = [
            'status' => 400,
            'message' => $error_message
        ];
    ...
    if (!preg_match($is_digit, $first_break) || !preg_match($is_digit, $second_break)) {
        $error_message = "Break time is required";
        $res = [
            'status' => 400,
            'message' => $error_message
    ...
    foreach ($data_sched['data_sched'] as $key => &$value) {
        ...
        $total_hrs = (strtotime($value['to_time']) - strtotime($value['from_time'])) / 3600;
        $sum_total_hrs = $sum_total_hrs + $total_hrs;
        ...
        $availability = array("fromtime" => $value['from_time'], "totime" => $value['to_time'], "teach_no" => 0, "break_time" => 0, "total_hrs" => $sum_total_hrs);
                ...
                for ($count = 0; $count < $day_wk; $count++) {
                    ...
                    foreach ($array_teaching[$index]['Day'] as $key_day => &$value_day) {
                        ...
                        if ($value_day['teach_no'] >= 3 and $value_day['break_time'] == 0) {
                            ...
                            $schedule_to = Schedule($value_day['fromtime'], $first_break);
                            $get_hours = (strtotime($schedule_to) - strtotime($value_day['fromtime'])) / 3600;
                            // insert Break Time
                            $break = "BREAK TIME";

                            $query = "INSERT INTO `temporary_sched`(`room_type`,`room`,`day`, `from_time`, `to_time`, `faculty_id`, `section`, `subject`,`course_title`,`unit`,`name`, `schedule`,`schedule_to`,`user_id`) VALUES ('$break','$break','$key_day','$from_time','$to_time','$faculty_id','$break','$break','$break','0','$name','$from_time','$schedule_to','$user_id')";
                        ...
                        if ($value_day['teach_no'] >= 8 and $value_day['break_time'] == 1) {
                            ...
                            $schedule_to = Schedule($value_day['fromtime'], $second_break);
                            $get_hours = (strtotime($schedule_to) - strtotime($value_day['fromtime'])) / 3600;
                        ...
                        while ($overlapping === true) {
                            ...
                            if (mysqli_num_rows($select_dup_run) > 0) { //if have overlap
                                ...
                                //add 03/13
                                if (strtotime($value_day['totime'] . ":00") == strtotime($schedule) || strtotime($value_day['totime'] . ":00") < strtotime($schedule_to) ||  strtotime($schedule_to) <= strtotime($schedule)) {
                                    $stop_loop = true;
                                    error_log('count_loop_3' . $value_day['totime'] . ":00" . " -- " . $schedule . "--" . $schedule_to);
                                    $overlapping = false;
                                    break;
                                }
                                //end 03/13
                                ...
                                if (strtotime($value_day['totime']) < $schedule_to_valid) {
                                    $stop_loop = true;
                                    $overlapping = false;
                                    error_log('count_loop_1' . $value_day['totime'] . ":00" . " -- " . $schedule . "--" . $schedule_to);
                                    break;
                                }
                                //end 03/13
                                ...
                                if (strtotime($value_day['totime'] . ":00") == strtotime($schedule) || strtotime($value_day['totime'] . ":00") < strtotime($schedule_to) ||  strtotime($schedule_to) <= strtotime($schedule)) {
                                    $stop_loop = true;
                                    error_log('count_loop_2' . $value_day['totime'] . ":00" . " -- " . $schedule . "--" . $schedule_to);
                                    $overlapping = false;
                                    break;
                                }

                                if (strtotime($schedule_to) > strtotime($value_day['totime'] . ":00") && $last_day_availability == $key_day) {
                                    $error_message = "No Room Found - . " . $faculty_id . " -" . $name . "</br> NOTE: .";
                        ...
                        //     $query = "SELECT `room`,`day`,`schedule`,`schedule_to` FROM `temporary_sched` WHERE `user_id` = '$user_id' AND `day` = '$key_day' AND section = '" . $section . "' AND (( '" . $schedule . "' > schedule && '" . $schedule_to . "' <  schedule_to ) 
                        // 	OR (('" . $schedule . "' > schedule && '" . $schedule . "' < schedule_to) || ('" . $schedule_to . "' > schedule && '" . $schedule_to . "' < schedule_to)) 
                        // 	OR ( '" . $schedule . "' = schedule || '" . $schedule_to . "' = schedule_to) 
                        // 	OR (schedule > '" . $schedule . "' && schedule_to < '" . $schedule_to . "'))";
                        //     if ($query_run = mysqli_query($db_connect, $query)) {
                        ...
                        // if ($no_room === true) {
                        //     $error_message = "Insufficient of room NOTE: you need to add rooms."."--".$room_value_type;
                        ...
                        $hash = sha1($room_type . $room_value_room . $key_day . $value_day['fromtime'] . $value_day['totime'] . $faculty_id . $section . $subject . $course_title . $unit . $name . $schedule . $schedule_to . $user_id);
                        $query_insert = "INSERT INTO `temporary_sched`(`room_type`,`room`,`day`, `from_time`, `to_time`, `faculty_id`, `section`, `subject`,`course_title`,`unit`,`name`, `schedule`,`schedule_to`,`user_id`,`total_hrs`) VALUES ('$room_type','$room_value_room','$key_day','" . $value_day['fromtime'] . "','" . $value_day['totime'] . "','$faculty_id','$section','$subject','$course_title','$unit','$name','$schedule','$schedule_to','$user_id','" . $total_hrs . "')";
