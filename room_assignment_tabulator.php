...
// WHERE MYSQL QUERY
// * FROM permanent_sched_detail WHERE room != 'BREAK TIME' AND parent_id = '$session_sched' AND `faculty_id` = '$faculty_id' ORDER BY id DESC
$id_faculty = "";
if($role == 'Faculty'){
	$id_faculty = "AND `faculty_id` = '$faculty_id'";
}
$sched = "`parent_id` = '$session_sched'";
...
$table_name= "permanent_sched_detail";
$field_query ='*';	
$pages =0;
$start = 0;
$size = 0;
...
$sorters =array();
$orderby ="id DESC";
$sql_where="$sched $id_faculty ";
$sql_conds="";
$sql_where_array=array();
$to_encode=array();
$output="";

$dbfield = array('parent_id','subject','section','schedule_from','schedule_to','day','room','unit','name'); // need iset based sa table columns
$dborig = array('parent_id','subject','section','schedule_from','schedule_to','day','room','unit','name'); // tabulator  checking
...
if(isset($_GET['filters'])){ // check yung filter
	...
	foreach($dborig as $id){
		if(isset($sort_filters[$id])){
			...
			if($id == $value){
				array_push($sql_where_array,'name LIKE \'%'.$value.'%\'');
				continue;
			}
...
$default_query ="SELECT ".$field_query." FROM ".$table_name."  ".$sql_conds."  AND `room` != 'BREAK TIME' AND `parent_id` = '$session_sched'  ORDER BY ".$orderby;
